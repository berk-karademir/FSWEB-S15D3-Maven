...
public class WordCounter {
    private static String text = "When the offensive resumed, the Turks received their first victory when the Greeks encountered stiff resistance in the battles of First and Second İnönü," +
    ...
    public static Map<String, Integer> calculatedWord() {
        ...
        // Kelimeleri ayırmak için boşluk ve noktalama işaretlerini kullanarak metni parçalayalım

        String[] words = text.toLowerCase().replaceAll("[^a-zA-Z0-9\\s]", "").split("\\s+");

        for (String word : words) {
            // Kelime harita içerisinde varsa sayısını artır, yoksa 1 olarak ekle
